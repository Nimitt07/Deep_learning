...
(from scratch). Produces:
  ...
  3) model output superimposed on target output (x on x-axis, y target/model on y-axis)
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "outputs", "regression_univariate")
LEARNING_RATE = 0.01
EPOCHS = 200
...
def run():
    ...
    X_train, y_train, X_test, y_test = load_univariate_regression()

    p = Perceptron(n_inputs=1, activation="linear", learning_rate=LEARNING_RATE, seed=10)
    errors = p.train(X_train, y_train, epochs=EPOCHS, seed=10)
    ...
    plt.title("Univariate Regression: Training Error vs Epochs")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    ...
    report_lines = [
        "Univariate Regression - Linear Perceptron",
        "=" * 50,
        f"Learned weight: {p.w[1]:.6f}, bias (w0): {p.w[0]:.6f}",
        "",
    ...
    # 3) Model output superimposed on target output (train and test)
    for split_name, X, y, pred in (
        ...
        plt.figure(figsize=(7, 5))
        plt.plot(X[order, 0], y[order], "o", markersize=3, label="Target output", alpha=0.6)
        plt.plot(X[order, 0], pred[order], "-", color="red", label="Model output")
        plt.xlabel("x")
        plt.ylabel("y")
        plt.title(f"Univariate Regression: Model vs Target Output ({split_name})")
        plt.legend()
        plt.tight_layout()
    ...
    for split_name, y, pred in (("train", y_train, pred_train), ("test", y_test, pred_test)):
        ...
        plt.title(f"Univariate Regression: Target vs Model Output ({split_name})")
        plt.legend()
        plt.tight_layout()
